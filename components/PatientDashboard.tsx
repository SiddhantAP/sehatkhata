...
import DescriptionsHistory from './DescriptionsHistory';

export default function PatientDashboard() {
  const { familyMembers, selectedFamilyMemberId, getMemberReports, getMemberTimeline, getMemberConsent, getMemberNotes, getMemberLatestNote, getMemberLatestAppointment, dismissAppointmentRecommendation, notifications, grantAccess, revokeAccess } = useAppState();
  ...
  const latestNote = getMemberLatestNote(member.id);
  ...
  return <div className="mx-auto max-w-3xl space-y-5 px-4 py-5">
    ...
    {(latestNote || latestAppointment) && <section><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">reminders</p><PatientReminders latestNote={latestNote} latestAppointment={latestAppointment} onDismissAppointment={dismissAppointmentRecommendation} /></section>}
    ...
    <section><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">descriptions from doctor</p><DescriptionsHistory notes={memberNotes} /></section>
